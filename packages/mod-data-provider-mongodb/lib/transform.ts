...
// This code has been copied from https://github.com/mongodb-js/jsonpatch-to-mongodb
// which is available under the MIT license

function toDot(path: string) {
  return path.replace(/^\//, '').replace(/\//g, '.').replace(/~1/g, '/').replace(/~0/g, '~');
}

/**
 * Transform a series of JSON patch operations into
 * a MongoDB update query.
 * @param patches The JSON path operations
 * @returns The update query
 */
export function transform(patches: Operation[]) {
  var update: any = {};
